...
import {
  renderAutoOpenCodeCliHelp,
  renderGeneralHelp,
  renderGuidedStartFlow,
} from "../copy/opencode-cli";
import {
  loadConfig,
  persistProjectModel,
  type AppConfig,
...
export function generalHelp(commandPrefix: string, managementCommandPrefix: string): string {
  return renderGeneralHelp(commandPrefix, managementCommandPrefix);
}
...
export async function executeAutoOpenCodeCliCommand(
  ...
  if (request.command === "help") {
    return {
      response: renderAutoOpenCodeCliHelp(autoProject.commandPrefix),
  ...
  if (request.command === "start") {
    return {
      response: renderGuidedStartFlow(autoProject.commandPrefix),
      projects: currentProjects,
    };
  }
